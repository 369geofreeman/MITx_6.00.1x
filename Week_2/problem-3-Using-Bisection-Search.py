...
# (in other words, the monthly payment is a multiple of $0.01). Does your code still work? It should, but you may notice that
# your code runs more slowly, especially in cases with very large balances and interest rates.
# (Note: when your code is running on our servers, there are limits on the amount of computing time each submission
# is allowed, so your observations from running this experiment on the grading system might be limited to an error message
...
# What we ultimately pay must be greater than what we would've paid in monthly installments, because the interest was c
# ompounded on the balance we didn't pay off each month. So a good upper bound for the monthly payment would be one-twelfth of the balance,
...
monthlyPaymentUpperBound = balance * (1 + monthlyInterestRate / 12.0)

while abs(newBalance) >= epsilon:
    newBalance = balance
    ans = (monthlyPaymentUpperBound + monthlyPaymentLowerBound) / 2
    for x in range(12):
        unb = newBalance - ans
        newBalance = unb * (1+monthlyInterestRate)
    if newBalance > 0:
